...
def detect(
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        # Get image path and original image
        ...
        save_path = str(Path(save_dir) / p.name)  # e.g., runs/detect/exp/image.jpg
        ...
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                # -----------------------------------
                # NEW: Extract and save face crops
                # -----------------------------------
                ...
                if face_crop.size > 0:
                    crop_path = Path(save_dir) / f"{p.stem}_face_{j}.jpg"
                    cv2.imwrite(str(crop_path), face_crop)
                    print("Saved face crop:", crop_path)
